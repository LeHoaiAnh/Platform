...
namespace Goap.Classes.Runners
{
    public class SensorRunner : ISensorRunner
    {
        ...
        {
            localWorldData = (LocalWorldData) agent.WorldData;

            localWorldData.Apply(worldData);
            
            foreach (var localWorldSensor in this.localWorldSensors)
            {
                localWorldData.SetState(localWorldSensor.Key, localWorldSensor.Sense(agent, agent.Injector));
            }
            
            foreach (var localTargetSensor in this.localTargetSensors)
            {
                localWorldData.SetTarget(localTargetSensor.Key, localTargetSensor.Sense(agent, agent.Injector));
            }
