...
namespace Hara.GUI
{
    public class PopupManager : MonoBehaviour
    {
        ...
        [HideInInspector] public List<FeatureType> newFearureTypes = new List<FeatureType>();
        [HideInInspector] public List<FeatureType> popupQueue;
        ...
        private void Start()
        {
            ...
            newFearureTypes = new List<FeatureType>();
        }
        ...
        public void CheckQueuePopup()
        {
            if(popupQueue.Count > 0)
            {
                popupQueue.Sort();
                ShowPopupInQueue(popupQueue[0]);
            }
        }

        private void ShowPopupInQueue(FeatureType feature)
        {
            switch (feature)
            {
                case FeatureType.ReviewApp:
                    
                    break;
                case FeatureType.UnlockEvent:

                    break;
                case FeatureType.UnlockTalent:

                    break;
                case FeatureType.UnlockMastery:

                    break;
                case FeatureType.PlusMastery:

                    break;
                case FeatureType.NewChapter:

                    break;
                case FeatureType.NewOffer:

                    break;
                case FeatureType.GrowthFund:

                    break;
            }
            popupQueue.Remove(feature);
        }
    }
}