    ...
    public class JsonMapper
    {
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                while (true)
                {
                    ...
                    if (t_data.Properties.ContainsKey(property))
                    {
                        ...
                        {
                            FieldInfo fieldInfo = ((FieldInfo)prop_data.Info);
        ...
        private static void RegisterBaseExporters()
        {
            ...
            base_exporters_table[typeof(DateTime)] =
                delegate (object obj, JsonWriter writer)
                {
                    string str = ((DateTime)obj).ToString(("yyyy-MM-ddTHH:mm:ss.fff"), CultureInfo.InvariantCulture);
                    writer.Write(str);
                };
                                        ...
                                        int depth)
        {
            ...
            if (obj is IJsonWrapper)
            {
                if (writer_is_private)
                    writer.WriteIJsonWrapper((IJsonWrapper)obj);
                else
            ...
            //if (obj is Hiker.Networks.Data.MatchNSlash.CardReward)
            //{
            //    var r = obj as Hiker.Networks.Data.MatchNSlash.CardReward;
            //    if (writer_is_private)
            //        writer.WriteIJsonWrapper(r.ToJsonData());
            //    else
            //        r.ToJsonData().ToJson(writer);
            //    return;
            //}
            ...
            foreach (PropertyMetadata p_data in props)
            {
                if (p_data.IsField)
                {
                    FieldInfo fieldInfo = ((FieldInfo)p_data.Info);
                    if (!fieldInfo.IsStatic)
                    {
                        object target_obj = fieldInfo.GetValue(obj);
                        if (target_obj != null) //PhuongTD
                        {
                            writer.WritePropertyName(p_data.Info.Name);
                            WriteValue(target_obj,
                ...
                else
                {
                    ...
                    if (p_info.CanRead)
                    {
                        object target_obj = p_info.GetValue(obj, null);
                        if (target_obj != null) //PhuongTD
                        {
                            writer.WritePropertyName(p_data.Info.Name);
                            WriteValue(target_obj,
        ...
        static bool ParseEnumAsString = false;
        public static string ToJson(object obj, bool enumAsString = false)
        {
            lock (static_writer_lock)
            {
                static_writer.Reset();

                ParseEnumAsString = enumAsString;

                WriteValue(obj, static_writer, true, 0);

                ParseEnumAsString = false;
                //if (shouldCompress)
                //{
